...
import { READING_EXERCISES } from '@drums/data/practiceLibrary'
  ...
  return (
    <div className="p-3 sm:p-4 md:p-6 lg:p-8 max-w-[1400px] mx-auto">
      ...
      <nav className="flex items-center gap-2 text-sm text-[#4b5563] mb-6">
        ...
        <span className="text-[#94a3b8]">Exercise Library</span>
      </nav>

      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-white tracking-tight mb-2">Exercise Library</h1>
        <p className="text-sm text-[#6b7280] leading-relaxed">
          Progressive reading exercises from whole notes to multi-bar phrases with fills.
          Covers 1-bar to 8-bar patterns at varying difficulty levels.
        </p>
      </div>
      ...
      <div className="space-y-2">
        {READING_EXERCISES.map((ex, i) => {
          ...
          const barCount = Math.ceil(ex.patternData.beats / ex.timeSignature[0])
          return (
            <button
              key={ex.id}
              onClick={() => navigate(`/drums/practice/play/${ex.id}`)}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="text-sm text-white font-medium">{ex.title}</div>
                <div className="text-xs text-[#6b7280] mt-0.5">{ex.description}</div>
